...
class RGB_LED():
    ...
    def set(self, R, G, B):					#if PWM, RGB values are 8 bit
        for i, color in enumerate([R,G,B]):
            if (self.PWM_Mode):
                dc = (255 - color) if self.polarity == 1 else color
                self.LED_Pins[i].duty_u16(int(dc/255 * self.PWM_MAX))
            else:
    ...
    def white(self):
        for LED in self.LED_Pins:						#turn on LED
            if (self.PWM_Mode):
                dc = 0 if (self.polarity == 1) else 255
                LED.duty_u16(int(dc/255 * self.PWM_MAX))
            else:
    ...
    def off(self):
        for LED in self.LED_Pins:						#turn off LED
            if (self.PWM_Mode):
                dc = 255 if (self.polarity == 1) else 0
                LED.duty_u16(int(dc/255 * self.PWM_MAX))
            else:
    ...
    def red(self, R):
        if (self.PWM_Mode):
            dc = 255 - R if (self.polarity == 1) else R
            self.LED_Pins[0].duty_u16(int(dc/255 * self.PWM_MAX))
        else:
            self.LED_Pins[0].value(R^self.polarity)
            
    def green(self, G):
        if (self.PWM_Mode):
            dc = 255 - G if (self.polarity == 1) else G
            self.LED_Pins[1].duty_u16(int(dc/255 * self.PWM_MAX))
        else:
            self.LED_Pins[1].value(G^self.polarity)
    
    def blue(self, B):
        if (self.PWM_Mode):
            dc = 255 - B if (self.polarity == 1) else B
            self.LED_Pins[2].duty_u16(int(dc/255 * self.PWM_MAX))
        else:            
